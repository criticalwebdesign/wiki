---
title: 0.0 Context
description: Introduction to the book and this wiki
We created this wiki as an additional resource to our book, Critical Web Design. Here you will find essential links from the book, especially in the "What You'll Make" sections for each chapter. You will also find additional images (overflow from what we wanted to print in the book) and exercises to take students/readers even further in their journey. This wiki is not a replica of the book, and should be used as a companion to reading and making. 
# draft: true
---

import CustomAside from '@/components/CustomAside.astro';

<CustomAside type="overview">{frontmatter.description}</CustomAside>






## Context

<figure>

![A screenshot of clickclickclick.click](@/assets/images/00/00-01-clickclickclick.png) 
The website, [clickclickclick.click](https://clickclickclick.click) is an example of critical web design. It presents a simple button when it loads. Then a recorded voice and text in the interface slowly informs the user that their actions are not only being recorded, but measured, scored, and represented as achievements. 


</figure>



## 0.1 How This Book Is Organized

1. Context
1. Modules
1. Active Learning
1. Teaching Guide
1. Chapter Summaries (see individual chapters)


## 0.2 Technical Requirements

1. Install Chrome Web browser: https://www.google.com/chrome
1. Create a Github account https://github.com
1. Create a Figma account and install the desktop app https://figma.com 
1. (optional) Install VS Code editor: https://visualstudio.microsoft.com/downloads


<figure>

![alt text](@/assets/images/00/00-03-agents-202307-compare-pie.png) 
[Wikimedia Foundation User Agent Breakdown](https://analytics.wikimedia.org/dashboards/browsers) by desktop (left) and mobile (right)

</figure>

<figure>

![Statcounter graph showing Chrome in the lead.](@/assets/images/00/00-04-agents-202307-statcounter.png)
Statcounter Browser Market Share Worldwide Jan 2009 - June 2023” 

</figure>


## 0.3 Conventions and Resources

Find all Critical Web Design book examples and exercise assets in the Github [repository](https://github.com/criticalwebdesign/book). Including...

- An [Errata](https://github.com/criticalwebdesign/book/blob/main/errata.md) with information about and submitting errors
- A [Wiki](https://criticalwebdesign.github.io/wiki/) with additional resources
- [Discussions](https://github.com/criticalwebdesign/book/discussions) where you can share your work and see what other readers have created. 

<figure>

![A twitter post naming several essential but fake web development tools](@/assets/images/00/00-05-popular-frameworks.png)
A tongue in cheek [post](https://twitter.com/johnnemann/status/1512034789313945604) on the plethora of and speed at which web frameworks change  

</figure>


## Aside Style Guide


<CustomAside type="excerpt">A short summary of each article.</CustomAside>


:::note[Author’s Note]
Standard callout for related info from our perspective. 
:::

:::tip[Pro Tip]
Standard callout for *advanced* related content. 
:::

:::tip[Continue Learning]
Standard section at bottom of a page about a subject
:::


:::caution
Warnings
:::

:::danger
Things to avoid.
:::
