...
5. With the API routes working you can change your project front end to use them instead. Open `public/assets/js/main.js` and change `url` inside `updatePassword()` to look like: 
...
Throughout the book you published your static web pages on Github Pages. This is a great (and free!) way to put your work in the world. However, GH does not allow you to run code in the back end. This means if you want to use a database, return custom response types (like JSON for our API), or do any other advanced scripting on the server, you need to use a real web host.
