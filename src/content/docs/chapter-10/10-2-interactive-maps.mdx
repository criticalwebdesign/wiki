...
In order to make the circle sizes appear constant relative to the interface we used [`L.circleMarker()`](https://leafletjs.com/reference.html#circlemarker-option) where you can define the radius in pixels (regardless of the map zoom level).
...
... instead of [`L.circle()`](https://leafletjs.com/reference.html#circle) which uses a static radius (meters).
