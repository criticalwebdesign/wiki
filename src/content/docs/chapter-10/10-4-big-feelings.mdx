...
3. Open `api/database/mongodb.js`. The database code is organized in its own file to “separate the concerns” and organize scripts by their specific tasks. This file connects to the database to let us save and retrieve data. Also notice that the database functionality (e.g. to create and insert data in the table) is exported inside an object called `db` to make it simpler to run the database queries from `routes.js`. 
...
4. Add the below function to `routes.js` (look for our finger emojis!) to retrieve data for the map. This route will use the `db.getAll()` function to query the database for all records. It uses `await` to make sure and wait for the query to finish so that it has results before sending a response. Test this route now in a new browser window by going to http://localhost:3000/api/feelings. You should see an empty array only because there is no data in the database, yet...
...
5. Add this endpoint below the code you added above to `routes.js` to add test data to the database. This route is not meant to be public, we are enabling it temporarily to add testing data only. Each time you load this page http://localhost:3000/api/addOneTest you will insert a new entry with random information. You can use the [Atlas Data Explorer](https://cloud.mongodb.com/) or the feelings endpoint to test it http://localhost:3000/api/feelings
...
2. We’ve already added a `/api/feeling` POST endpoint that receives the request from the form and updates the database. Look inside `routes.js` and you will notice it uses `router.post()` instead of `router.get()`. 
